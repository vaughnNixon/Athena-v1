...
### 2. Next-Generation Chunk Memory Engine
- **Chronological Chunking**: Groups conversation context into contiguous chronological blocks for accurate temporal reasoning and timeline tracking.
- **Normalized Keyword Lookup (`chunk_keywords`)**: Normalizes search keywords into a dedicated index table, enabling `O(log K)` search speeds across millions of memory items.
...
- **Extensible Schema**: Schema metadata tracks deprecated and active tables. Metadata columns reserve `"workspace"`, `"project"`, `"skill"`, and `"annotation"` scopes for future skill-specific memory modules.
